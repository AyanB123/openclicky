...
final class OpenClickyNotchCaptureWindowManager {
    ...
    private func hideMainPanel(restoresStatusSurface: Bool = true) {
        ...
        if restoresStatusSurface {
            restoreStatusSurfaceAfterMainPanelClosed()
        } else if activeMode == .collapsedText {
            startCollapsedHoverProbe()
        }
    }

    private func restoreStatusSurfaceAfterMainPanelClosed() {
        guard mainPanel?.isVisible != true else { return }

        switch activeMode {
        case .collapsedText:
            guard let submitText = persistentSubmitText else { return }
            // Opening the main panel hides the compact status surface so it does
            // not sit under the dropdown. When the dropdown closes, restore the
            // external no-notch fallback immediately instead of waiting for the
            // next hover probe or voice-state update.
            collapseToPill(accentColor: persistentAccentColor, submitText: submitText)
        case .voice:
    ...
    private func updateForegroundAppIcon(bundleIdentifier: String?, bundlePath: String?, appIcon: NSImage?, name: String) {
        ...
        if activeMode == .collapsedText, mainPanel?.isVisible != true, panel?.isVisible != true, !isUsingDynamicNotchKitStatusSurface {
            restoreStatusSurfaceAfterMainPanelClosed()
        }
    }
